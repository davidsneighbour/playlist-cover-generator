...
* Auto-save (when the `autoSave` prop is on, the default) restores the last session from localStorage at mount and writes it back on a 500ms debounce. Initial state precedence is `DEFAULT_STATE` < restored < `initialState` prop, so an explicit `initialState` still wins per key. The serialize/parse helpers and the versioned `STORAGE_KEY` live in [src/lib/storage.js](src/lib/storage.js) (tested); writes fall back to dropping `backgroundImageData` if the full payload exceeds the storage quota, and a malformed payload is ignored.
