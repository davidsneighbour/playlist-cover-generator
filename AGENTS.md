# Agent guide

Instructions for AI coding agents working in this repository. This is the canonical source; tool-specific config files (for Claude Code, Cursor, Copilot, Gemini, and Windsurf) point back here.

## What this project is

An embeddable React component (`ImageGenerator`) for generating images from templates: album covers, social media posts, banners, and more. A user uploads a background image, adds draggable text/image/shape layers, optionally shows a snap grid, and exports to PNG, SVG, or a re-loadable JSON project. The canvas is plain SVG, kept clean so exported files open in other editors. A programmatic API (`buildStateFromTemplate`, `generateFromTemplate`) lets external programs supply inputs and receive a generated PNG without opening the editor.

## Tech stack

* **React 19** with hooks only (no class components).
* **Vite 6** for dev server and build.
* **Tailwind CSS 4** via the `@tailwindcss/vite` plugin (no `tailwind.config.js`; configuration is CSS-first in [src/index.css](src/index.css)).
* JavaScript with JSX (`.jsx`). There is no TypeScript in this project.
* **Headless UI** (`@headlessui/react`) for the accessible dialog and disclosure transitions, and **Lucide** (`lucide-react`) for icons.

## Commands

| Command | What it does |
| --- | --- |
| `npm install` | Install dependencies. |
| `npm run dev` | Start the Vite dev server. |
| `npm run build` | Demo app build into `dist/`. |
| `npm run build:lib` | Library (npm package) build into `dist/`. |
| `npm run preview` | Serve the production build. |
| `npm test` | Run the Vitest suite once. |
| `npm run test:watch` | Run Vitest in watch mode. |
| `npm run lint` | Lint with ESLint (react-hooks and jsx-a11y). Run in CI. |
| `npm run lint:fix` | Lint and auto-fix what is fixable. |
| `npm run format` | Format with Prettier (opt-in; not enforced in CI). |
| `npm run format:check` | Check formatting without writing. |

Always run `npm run lint`, `npm run build`, and `npm test` after non-trivial changes to confirm the project still lints, compiles, and the suite passes. CI ([.github/workflows/ci.yml](.github/workflows/ci.yml)) runs lint, tests, and both builds on push and pull request. ESLint is configured in [eslint.config.js](eslint.config.js); the `jsx-a11y` rules that conflict with documented design decisions (self-labelled inline controls, non-interactive layer-row wrappers) are kept as warnings, not errors.

## Architecture

The whole editor lives in [src/components/CoverGenerator.jsx](src/components/CoverGenerator.jsx). Read it before changing anything. Key pieces:

* `state.canvasWidth`/`state.canvasHeight` define the internal SVG coordinate space (default 600×600). The rendered size scales via a `ResizeObserver` that preserves the canvas aspect ratio; all geometry is stored in canvas units, not screen pixels. The export resolution is independent: `state.exportWidth`/`state.exportHeight` size the exported PNG and the SVG's `width`/`height` attributes, while the `viewBox` stays `0 0 canvasWidth canvasHeight` so output scales cleanly. Keep new geometry in canvas-unit space; do not key it off export dimensions. `CANVAS_SIZE` (600) is kept as a backward-compat constant but is no longer the only valid canvas size.
* `CoverGenerator` (also exported as `ImageGenerator`) is the primary component. It owns all state and notifies the host through `onStateChange` (fired from an effect whenever state changes, skipping the initial mount).
* Share links encode the layout into the URL hash (`#s=...`). `encodeShareState`/`decodeShareState`/`readShareToken` ([src/lib/share.js](src/lib/share.js), tested) use UTF-8-safe base64url and drop `backgroundImageData` so the link stays a reasonable length (the recipient supplies their own image). At mount a present share token wins over the localStorage restore but still loses to an explicit `initialState`: precedence is `DEFAULT_STATE` < restored < shared < `initialState`. This is client-only; there is no server "cloud save".
* Auto-save (when the `autoSave` prop is on, the default) restores the last session from localStorage at mount and writes it back on a 500ms debounce. Initial state precedence is `DEFAULT_STATE` < restored < `initialState` prop, so an explicit `initialState` still wins per key. The serialize/parse helpers and the versioned `STORAGE_KEY` live in [src/lib/storage.js](src/lib/storage.js) (tested); writes fall back to dropping `backgroundImageData` if the full payload exceeds the storage quota, and a malformed payload is ignored. A small `saveStatus` indicator under the canvas shows "Saving…" while the debounce is pending and "Saved" once it lands (skipped on first mount so it does not flash; only shown when `autoSave` is on).
* `useHistoryState` wraps the single state object to provide undo and redo. Every mutation goes through its `commit(patch, coalesceKey)`. Discrete edits push a new history entry; rapid edits that share a `coalesceKey` (dragging a text, typing in a field) collapse into one step. Undo and redo are bound to Cmd/Ctrl+Z and Cmd/Ctrl+Shift+Z (and Ctrl+Y), and ignored while a form field is focused so native text undo still works. When adding a new continuous interaction, give it a stable `coalesceKey`; for one-off actions, omit it.
* `SVGCanvas` ([src/components/SVGCanvas.jsx](src/components/SVGCanvas.jsx)) renders the SVG tree in paint order: the gradient background, the background `image`, the color overlay, image layers, shapes, the grid, then text, and finally the selection outline and resize handles (both `data-layer="selection"`, stripped on export). It is extracted from `CoverGenerator.jsx` so it can be used by the headless `generateFromTemplate` path via `react-dom/server.renderToString`; pass `null` for all selection/handler props in that context.
* `GridOverlay` draws minor and major lines. It is marked `data-layer="grid"` so exporters can strip it.
* Rulers (`TopRuler`/`LeftRuler`, tick values from `rulerTicks` in [src/lib/rulers.js](src/lib/rulers.js)) are optional chrome rendered as separate SVGs *outside* the exported canvas SVG, so they never appear in PNG/SVG output. Their visibility is local UI state (`showRulers`), not part of the persisted document, so toggling them is not undoable and not saved to JSON. The canvas-fit `ResizeObserver` watches the canvas *column* (whose width comes from the page layout, not its children) and subtracts the ruler thickness, so showing rulers shrinks the canvas instead of overflowing.
* The controls are an accordion of `CollapsibleCard`s ([src/components/Accordion.jsx](src/components/Accordion.jsx)) driven by the pure reducer in [src/lib/accordion.js](src/lib/accordion.js) (one unpinned card open at a time; pinned cards stay open). `CoverGenerator` owns the accordion state and provides it via `AccordionContext`; selecting or adding a layer calls `openCard` for that layer's properties card (ids `props-text`/`props-image`/`props-shape`), and adding also scrolls to it. The canvas column is sticky on `lg`. Card order is layer-first: the `layers` overview (default open) is at the top, then the layer-creating/-editing cards (`background`, `overlay`, `text-layers` + `props-text`, `image-layers` + `props-image`, `shapes` + `props-shape`), with the utilities (`template`, `history`, `fonts`, `grid`, `export`) below them.
* The `layers` card is a unified, front-to-back overview of everything on the canvas. `buildLayerList(state, selection)` ([src/lib/layerList.js](src/lib/layerList.js), tested) returns one entry per text/shape/image layer (front-most first, with `id`, `selected`, an `icon` key, and the `hidden`/`locked` flags) plus the `overlay` and `background` singletons (always listed, `muted` when disabled/empty; a hidden layer is also `muted`). The component renders each through `LayerRow` — the `icon` string maps to a Lucide glyph via `LAYER_ICONS`, the label button calls `goToLayer`, which selects the layer (text/image/shape, which opens its properties card) or opens the `background`/`overlay` card, then scrolls it into view. Each real layer row also has lock and visibility toggle buttons (`toggleLayerLock`/`toggleLayerVisibility`, which set the per-layer `locked`/`hidden` flags by id, one undo step each). A `hidden` layer is not rendered on the canvas (so it is excluded from exports too); a `locked` layer renders but has `pointer-events: none`, no drag/resize handles, and is skipped by keyboard nudge and delete — toggle it from the panel to edit it again. The lib stays DOM-free; icons and click wiring live in the component.
* Pointer dragging is the shared `useSvgDrag` hook, used by both `TextElement` and `ImageElement`. It converts screen coordinates to SVG coordinates through `getScreenCTM().inverse()`, then snaps and clamps. Do not reimplement drag math with raw offsets; the matrix transform keeps it correct under scaling.
* Shape primitives (`state.shapes`) render between the image layers and the grid. They share the bounding-box model (x, y, width, height), drag hook, and z-order helpers used by image layers. The `type` is one of `SHAPE_TYPES` (`rect`, `circle`, `triangle`): a `circle` is an `<ellipse>` fit to the box, a `triangle` is a `<polygon>` whose points come from `trianglePoints` (apex top-center, base across the bottom), and a `rect` takes an optional corner `radius` clamped by `cornerRadius` to half the smaller side (see [src/lib/shapes.js](src/lib/shapes.js)). Text, image, and shape selection are mutually exclusive (`selectText`/`selectImage`/`selectShape`).
* Image layers (logos, overlays) live in `state.images` and render between the background and the grid. Their factory and blend-mode/fit helpers are in [src/lib/images.js](src/lib/images.js); z-order reuses the generic `reorder`/`bringToFront`/`sendToBack` helpers from layers.js since they key off `id`. Layer `data` (a data URL) is kept in exported JSON so image layers round-trip (unlike the background, which is stripped).
* The background image is cropped by `state.backgroundTransform` (`{ zoom, panX, panY }`): `backgroundCrop` ([src/lib/background.js](src/lib/background.js), tested) turns it into an `x/y/width/height` rect rendered with `preserveAspectRatio="none"` (zoom 1 + centered reproduces the old `xMidYMid slice`). It needs the image's natural size, probed on upload into `backgroundNaturalWidth`/`backgroundNaturalHeight`; when those are absent (e.g. an image set via `initialState`) it falls back to `slice`. Uploading a new image resets the transform to centered cover.
* Background filters (`state.backgroundFilters`: `{ brightness, contrast, saturate, blur }`) render as an SVG `<filter id="bg-filter">` (so they survive PNG/SVG export, unlike a CSS `filter`) referenced by the background `<image>`. Brightness and contrast fold into one `feComponentTransfer` via `brightnessContrastTransfer`, saturation is an `feColorMatrix type="saturate"`, and blur is an `feGaussianBlur`. `isFilterActive` skips the whole filter when nothing is changed. Helpers and defaults are in [src/lib/filters.js](src/lib/filters.js) (tested).
* The gradient background (`state.backgroundGradient`) is a two-stop linear/radial gradient painted by `GradientBackground` as the bottom-most layer, beneath the background image, so it shows when no image is loaded (an opaque image covers it). Its defaults and type guard are in [src/lib/background.js](src/lib/background.js) (tested); it reuses `gradientVector` for the linear axis. The Background card also has a "Remove image" button so the gradient can be revealed.
* The color overlay (`state.overlay`) is a single full-canvas fill painted by `ColorOverlay` immediately above the background and below every other layer, for text legibility. It is `solid` (one color plus opacity) or a two-stop `linear`/`radial` gradient whose stops carry their own alpha; an optional `blendMode` applies to the overlay rect (so `multiply` darkens only the background beneath). Pure helpers (`DEFAULT_OVERLAY`, `OVERLAY_TYPES`, `gradientVector`) live in [src/lib/overlay.js](src/lib/overlay.js) and are tested; `gradientVector(angle)` returns the linear axis endpoints in objectBoundingBox units (0 = top→bottom, 90 = left→right). The overlay is part of the artwork, so it is kept in exports and JSON.
* `snapValue` rounds a coordinate to the grid when snapping is on. Reuse it rather than duplicating rounding logic.
* Text-layer z-order is the `texts` array order: index 0 is painted first (bottom) and the last item is painted last (front). The pure reorder helpers (`reorder`, `bringToFront`, `sendToBack`, `displayIndexToArrayIndex`, and `duplicateById`) live in [src/lib/layers.js](src/lib/layers.js) and return the same array reference on a no-op so reorders never add empty undo entries. `duplicateById(layers, id, makeCopy)` inserts a copy right after the original (one step in front); the per-type handlers (`duplicateText`/`duplicateImage`/`duplicateShape`) supply a `makeCopy` that assigns a fresh id, offsets by `DUP_OFFSET`, and deep-copies nested objects (text `shadow`). The layer list is displayed front-to-back, so the UI converts display positions to array indices with `displayIndexToArrayIndex`.
* Text presentation helpers live in [src/lib/text.js](src/lib/text.js). `textStrokeAttrs` turns a layer's `stroke`/`strokeWidth` into SVG attributes (no stroke when width is 0, otherwise `paint-order: stroke` so the outline sits under the fill); spread its result onto the `<text>` element. `textShadowFilter` resolves a layer's `shadow` object (`{ color, blur, dx, dy }`, or null when off) into params for an SVG `feDropShadow`; the canvas renders one `<filter>` per shadowed layer in `<defs>` and the `<text>` references it. Multi-line text: `content` may contain `\n`; `textLines` splits it into lines the canvas renders as one `<tspan>` each (all sharing the layer's `x`, every line after the first offset by `lineHeightEm(text)` em), and `lineHeight` (a multiple of the font size, default 1.2) is the per-line `dy`. New text layers carry `stroke`, `strokeWidth`, `shadow`, and `lineHeight`, all part of `TEXT_KEYS` — keep them in sync across `addText`, the templates, and that list.
* Custom (Google) fonts are tracked in `state.fonts`. For the live canvas the component injects a stylesheet `<link>` per font (`googleFontCssUrl`). The Fonts panel typeahead and export embedding share one cached fetch of the Google Fonts Developer API catalog (`loadFontCatalog`, needs `googleFontsApiKey`). On export, `embedFontsInClone` inlines only the weights/styles actually used as base64 `@font-face`, fetching the catalog's gstatic file URLs (CORS-enabled, unlike the CSS endpoint). The pure helpers in [src/lib/fonts.js](src/lib/fonts.js) (`googleFontsListUrl`, `filterFontNames`, `fontVariantKey`, `variantFontFace`, `pickVariantFile`, `buildFontFaceRule`) are tested; the network/DOM work stays in the component and is best-effort (export still succeeds with a fallback if a fetch fails).
* Right-clicking a layer opens `ContextMenu` (duplicate, bring to front, send to back, delete). The `<svg>`'s `onContextMenu` is delegated: it walks up from the event target to the nearest element carrying `data-text-id`/`data-image-id`/`data-shape-id`, selects that layer, and opens the menu at the cursor; right-clicking the background or empty canvas falls through to the native menu. The menu clamps into the viewport with the pure `clampMenuPosition` ([src/lib/menu.js](src/lib/menu.js), tested) and closes on an outside mousedown, Escape, scroll, or after an action. Its action list is resolved from the same per-type handlers used elsewhere, so behavior stays consistent.
* A second keydown effect handles layer editing when a layer is selected: Delete/Backspace removes it (text/image/shape, whichever is selected), and the arrow keys nudge it by 1px (or `grid.spacing` with Shift). It is guarded the same way as undo/redo (ignored in form fields) plus while the help overlay is open, and only acts when something is selected so plain arrows still scroll otherwise. Nudges are coalesced per layer (key `nudge-<ids>`) so a burst is one undo step. The pure `nudgeDelta`/`isDeleteKey` helpers are in [src/lib/shortcuts.js](src/lib/shortcuts.js) and tested. This effect lives after the delete handlers it calls (they are `const`, so referencing them earlier would hit the temporal dead zone).
* Accessibility: an `aria-live="polite"` region (`sr-only`) announces layer add/delete/duplicate and undo/redo via `announce(msg)`; messages come from `actionAnnouncement`/`layerNoun`/`describeLayer` in [src/lib/a11y.js](src/lib/a11y.js) (tested). The region re-announces a repeated message by flipping a trailing space keyed off a nonce. Each layer-list row's label is a real `<button aria-pressed>` so layers are selectable from the keyboard (the outer row keeps a mouse `onClick`; it has no ARIA role, so there is no nested-interactive violation). The canvas elements themselves are also focusable (`tabIndex={0}`, `role="button"`, `aria-label` from `describeLayer`, `aria-pressed`) and select on Enter/Space, with a `:focus-visible` outline in [src/index.css](src/index.css); the `<svg>` is a labelled `role="group"`. Escape deselects (handled in the layer-editing keydown effect, which is inert while the help dialog or context menu is open). Icon-only buttons carry an `aria-label` in addition to their `title`, the inline sliders/selects/colour pickers carry `aria-label`s, and `NumberInput` associates its `<label>` via `htmlFor`/`id`. `.btn-primary` uses blue-600 (not the blue-500 accent) so its white label meets AA contrast; blue-500 is reserved for non-text accents (selection/focus rings). `text-gray-400` is below AA and is used only for non-essential hints — see DESIGN.md. `doUndo`/`doRedo` wrap the history actions to announce them.
* The help overlay (`HelpDialog`) is a Headless UI `Dialog` showing the app version (imported from `package.json`), the keyboard shortcuts, and mouse tips. F1 toggles it (handled in the keydown effect, before the input-focus guard so it works anywhere); Escape, an outside click, and the X all close it. The shortcut list and the platform-aware key formatter (`SHORTCUTS`, `formatKeys`) live in [src/lib/shortcuts.js](src/lib/shortcuts.js) and are tested; `'mod'` renders as Cmd on macOS and Ctrl elsewhere. It is transient UI, not part of the document.
* Templates are plain layout data in [src/lib/templates.js](src/lib/templates.js). Each template carries `id`, `name`, `category`, `description`, `canvasWidth`, `canvasHeight`, `exportWidth`, `exportHeight`, `fields[]` (named input bindings for the programmatic API), and a `layout` object with `texts`, `grid`, `snapToGrid`, and optionally `overlay`. `instantiateTemplate` builds editor state from a template: it keeps the current background image, deep-copies grid/overlay, applies canvas/export dimensions, and assigns each text a fresh id from a `makeId` callback. Applying a template in the editor goes through `update`, so it is one undoable step. Add new layouts to the `TEMPLATES` array; give every text layer all of `TEXT_KEYS`. See [TEMPLATE-AUTHORING.md](TEMPLATE-AUTHORING.md) for the full schema and authoring guide.
* The programmatic API (`buildStateFromTemplate` in [src/lib/templateApi.js](src/lib/templateApi.js)) is a pure function that accepts a template id or object and a named inputs map (`backgroundImageData`, plus any fields named in `template.fields`). It returns a complete state object, safe to use in Node.js. Browser PNG generation is in [src/lib/generate.js](src/lib/generate.js); Node.js PNG generation is in [src/generate.node.js](src/generate.node.js) (requires `@resvg/resvg-js` optional peer dep).

### State shape

The single state object is the contract for JSON import/export and the `initialState` prop. Keep it stable:

```js
{
  backgroundImage: string | null,      // file name, for display only
  backgroundImageData: string | null,  // data URL; excluded from JSON export
  backgroundNaturalWidth: number | null,   // probed on upload, for the crop
  backgroundNaturalHeight: number | null,
  backgroundTransform: { zoom, panX, panY },// background crop/pan/zoom
  backgroundFilters: { brightness, contrast, saturate, blur },
  backgroundGradient: { enabled, type, color, color2, angle },
  texts: [
    { id, content, x, y, fontSize, fontFamily, color, bold, italic, anchor, stroke, strokeWidth, shadow, lineHeight, hidden?, locked? }
  ],
  images: [
    { id, name, data, x, y, width, height, opacity, blendMode, naturalWidth, naturalHeight, lockAspect, hidden?, locked? }
  ],
  shapes: [
    { id, type, x, y, width, height, fill, stroke, strokeWidth, opacity, radius, hidden?, locked? }
  ],
  overlay: { enabled, type, color, opacity, color2, opacity2, angle, blendMode },
  grid: { enabled, spacing, majorEvery },
  snapToGrid: boolean,
  fonts: [ string ],                   // custom (Google) font family names
  canvasWidth: number,                 // internal SVG coordinate width (default 600)
  canvasHeight: number,                // internal SVG coordinate height (default 600)
  exportWidth: number,                 // export pixel width (PNG + SVG width attribute)
  exportHeight: number,                // export pixel height
}
```

`backgroundImageData` is deliberately omitted from exported JSON to keep project files small; importing JSON restores the layout over whatever image is loaded next. Preserve this behavior.

### Export rules

* PNG and SVG exporters clone the live SVG, then call `stripExportArtifacts(clone)` ([src/lib/export.js](src/lib/export.js), tested) to remove the editor-only `[data-layer="grid"]` and `[data-layer="selection"]` elements, and clear interaction-only styles. The grid and selection chrome must never appear in exports; reuse the helper rather than re-listing the selector.
* PNG rasterizes at `state.exportWidth × state.exportHeight` (via `exportScale(exportWidth, canvasWidth)` and `exportScale(exportHeight, canvasHeight)` from [src/lib/canvas.js](src/lib/canvas.js)); SVG sets its `width`/`height` attributes to the export dimensions and keeps the `viewBox` at `0 0 canvasWidth canvasHeight`. PNG rasterization is shared through `svgCloneToPngBlob` (prepare the clone, then it serializes, loads, and draws to a canvas), used by both single and batch export.
* Batch export clones the live SVG once per uploaded image, swaps that image in as the `data-layer="background"` element (creating one after the gradient/defs when there is no current background), crops it with `backgroundCrop` and the current transform/filter, rasterizes via `svgCloneToPngBlob`, and packs the PNGs with the dependency-free STORE-method ZIP writer (`buildZip`/`crc32` in [src/lib/zip.js](src/lib/zip.js), tested). SVG must stay valid and editable in external tools, so avoid embedding interaction handlers or editor-only attributes in the serialized output.

## Conventions

* Match the existing style in [src/components/CoverGenerator.jsx](src/components/CoverGenerator.jsx): functional components, `useCallback` for handlers passed as props, and small helper components within the file.
* Styling is Tailwind utility classes plus the shared `.btn-primary`, `.btn-secondary`, and `.input` classes defined in [src/index.css](src/index.css). Reuse those classes instead of repeating utility chains.
* Icons come from `lucide-react`. The `.btn-*` classes are `inline-flex items-center justify-center gap-1.5`, so a leading icon before the label aligns automatically; use `h-4 w-4` for icons in labeled buttons and `h-3.5 w-3.5` for icon-only controls (layer rows). Icon-only buttons keep their `title` and `aria-label`; decorative icons get `aria-hidden`.
* Follow [DESIGN.md](DESIGN.md) for color, typography, and spacing. The theme is light, minimal, and content-first.
* Keep the component dependency-free where reasonable. Do not add a heavy SVG-editor library; the value here is clean, portable SVG.
* The package ships as a library via `npm run build:lib` ([vite.config.lib.js](vite.config.lib.js)): entry [src/index.js](src/index.js), ESM + CJS output, with `react`, `react-dom`, `react/jsx-runtime`, `@headlessui/react`, and `lucide-react` left external (React/React-DOM are peer dependencies, Headless UI and Lucide regular ones). The default `npm run build` still builds the demo app. If you add a runtime dependency, decide deliberately whether it should be bundled, a peer, or external, and update both the lib config's `external` list and `package.json`.

### Tests

Tests run on Vitest. Prefer extracting non-trivial logic into pure functions under [src/lib/](src/lib/) and testing those directly, rather than driving the DOM; SVG drag and HTML drag-and-drop are hard to test headlessly, but the math behind them is not. Tests live in [tests/](tests/), named `<module>.test.js`, and import the module under test from the matching path under `../src/lib`. Add or update tests for any new pure logic.

The default environment is Node. For the few tests that need a DOM, add the `// @vitest-environment jsdom` pragma at the top of the file. [tests/CoverGenerator.test.jsx](tests/CoverGenerator.test.jsx) renders the whole component with `@testing-library/react` to cover wiring that has no pure-function seam — add a text layer, undo/redo it, deselect on Escape, and the form-field shortcut guard. jsdom has no SVG layout or `ResizeObserver`, so that file stubs `getBBox`, `scrollIntoView`, and `ResizeObserver`; reuse those stubs for new DOM tests.

### Markdown

This repository lints markdown with `markdownlint-cli2` using the config in `.markdownlint-cli2.jsonc`. When editing any `.md` file, follow these rules so it passes:

* Headings use sentence case, not title case (for example, "Getting started", not "Getting Started").
* Unordered lists use `*`, nested two spaces.
* Bold uses `**`, italics use `*`.
* Horizontal rules use `---`.
* Tables are compact (single spaces around cell content) with leading and trailing pipes.
* Use a real ellipsis character, not three dots, and straight quotes, not curly quotes.
* Do not use generic link text such as "click here", "here", "link", or "more".

## Do and do not

* Do keep all geometry in canvas units and convert at the edges only.
* Do update [todo.md](todo.md) when you defer work, and [DESIGN.md](DESIGN.md) when you introduce new visual tokens.
* Do verify with `npm run build` before declaring a change done.
* Do not commit or push unless explicitly asked.
* Do not introduce TypeScript, a CSS framework other than Tailwind, or a `tailwind.config.js` file.
