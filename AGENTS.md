...
* The `layers` card is a unified, front-to-back overview of everything on the canvas. `buildLayerList(state, selection)` ([src/lib/layerList.js](src/lib/layerList.js), tested) returns one entry per text/shape/image layer (front-most first, with `id`, `selected`, and an `icon` key) plus the `overlay` and `background` singletons (always listed, `muted` when disabled/empty). The component renders each through `LayerRow` — the `icon` string maps to a Lucide glyph via `LAYER_ICONS`, and clicking calls `goToLayer`, which selects the layer (text/image/shape, which opens its properties card) or opens the `background`/`overlay` card, then scrolls it into view. The lib stays DOM-free; icons and click wiring live in the component.
...
```js
{
  ...
  texts: [
    { id, content, x, y, fontSize, fontFamily, color, bold, italic, anchor, stroke, strokeWidth, shadow, lineHeight }
  ],
  images: [
