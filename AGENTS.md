...
* `SVGCanvas` renders the SVG tree in paint order: the gradient background, the background `image`, the color overlay, image layers, shapes, the grid, then text, and finally the selection outline and resize handles (both `data-layer="selection"`, stripped on export).
